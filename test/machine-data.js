...
describe('machine-generated extension data (exported by the module)', () => {
  ...
  it('sets a `slug` property on every extension', () => {
    expect(extensions.every((extension) => extension.slug.length > 0)).to.equal(true)
  })
  ...
  describe('releases', () => {
    const extensionsWithRepos = require('../lib/extenions-with-github-repos')
    const extensionsWithLatestRelease = extensions.filter((extension) => extension.latestRelease)

    it('tries to fetch a release for every extension with a GitHub repo', () => {
      expect(extensions.filter((extension) => extension.latestReleaseFetchedAt).length).to.equal(
    ...
    it('sets `latestRelease` on extensions with GitHub repos that use Releases', () => {
      expect(extensionsWithLatestRelease.every((extension) => extension.latestRelease)).to.eq(
        true
      )
    })
    ...
    it('sets `latestReleaseFetchedAt`', () => {
      expect(
        extensionsWithLatestRelease.every((extension) => extension.latestReleaseFetchedAt)
      ).to.eq(true)
    })
  })
  ...
  it('rewrites relative image source tags', () => {
    const beaker = extensions.find((extension) => extension.slug === 'beaker-browser')
